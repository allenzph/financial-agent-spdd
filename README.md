...
### Pull Ollama models (canonical path)

The project defaults to local Ollama. Pull the models listed in
`.env.example`:
...
Skipping this step is fine if you plan to use `LLM_PROVIDER=openrouter`
instead (set `OPENROUTER_API_KEY` in your `.env`).

### Run checks

```bash
poetry run ruff check .
poetry run mypy --strict --explicit-package-bases app data_pipelines
poetry run pytest -v
```
